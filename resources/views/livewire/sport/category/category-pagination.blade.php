<div class="bg-white shadow rounded-xl">
    ...
    <div class="flex flex-row justify-between items-center border-b-2 border-zinc-200 py-4 px-4">
        <div>
            <h4 class="text-2xl text-zinc-600 leading-6 font-bold">
                ...
                <span>Categories ({{ $search != '' ? $found : $total }})</span>
            </h4>
        </div>
        <div>
    ...
    <div class="flex flex-col justify-between sm:flex-row items-center p-4 w-100 sm:w-100 gap-2 sm:gap-4">
        ...
        <div class="relative w-full">
            ...
            <input type="search" class="w-full rounded-lg pl-14 placeholder-zinc-400 focus:outline-none focus:ring-0 focus:border-orange-500 border-2 border-zinc-200" placeholder="Search by category name" wire:model.live="search">
        </div>
        <!-- Pagination -->
    ...
    @if (count($selections) > 0)
        <div class="flex flex-row justify-start items-end sm:flex-row sm:justify-start gap-6 py-0 px-6">
            <span class="text-sm font-semibold">categories Selected</span>
        ...
        <div class="overflow-x-auto">
            <div class="min-w-full inline-block align-middle">
                ...
                <div class="overflow-hidden">
                            ...
                            <tbody class="divide-y divide-zinc-200">
                                ...
                                @foreach ($categories as $category)
                                    <tr class="even:bg-zinc-200 odd:bg-white transition-all duration-1000 hover:bg-yellow-400">
                                        <td class="p-4 whitespace-nowrap text-md text-center leading-6 font-medium text-gray-900"><input wire:model.live="selections" type="checkbox" class="text-green-600 outline-none focus:ring-0 checked:bg-green-500" value={{ $category->id }}></td>
                                        <td class="p-4 whitespace-nowrap text-md leading-6 font-medium text-gray-900">{{ $category->id }}</td>
                                        <td class="p-4 whitespace-nowrap text-md leading-6 font-medium text-gray-900">{{ $category->name }}</td>
                                        <td class="p-4 whitespace-nowrap text-md leading-6 font-medium text-gray-900">{{ date('d-m-Y', strtotime($category->created_at)) }}</td>
                                        <td class="p-4 whitespace-nowrap text-md leading-6 font-medium text-gray-900">{{ date('d-m-Y', strtotime($category->updated_at)) }}</td>
                                        <td class="p-4">
                                            <div class="flex justify-center items-center gap-4">
                                                <!-- Entries for this category -->
                                                <a href="{{ route('sportcategory.entries', $category) }}" title="See Entries for this category">
                                                    <span class="text-orange-600 hover:text-black transition-all duration-500"><i class="fa-lg fa-solid fa-basketball"></i></span>
                                                </a>
                                                <!-- Show -->
                                                <a href="{{ route('sportcategory.show', $category) }}" title="See this category">
                                                    <span class="text-green-600 hover:text-black transition-all duration-500"><i class="fa-lg fa-solid fa-circle-info"></i></span>
                                                </a>
                                                <!-- Edit -->
                                                <a href="{{ route('sportcategory.edit', $category) }}" title="Edit this category">
                                                ...
                                                <form action="{{ route('sportcategory.destroy', $category) }}" method="POST">
                                                    ...
                                                    @method('DELETE')
                                                    <button onclick="return confirm('Are you sure you want to delete the category: {{ $category->name }}?')" title="Delete this category">
                    ...
                    @else
                        <div>
                            <span class="text-lg text-red-600 px-4">No categories found</span>
                        </div>
                    @endif
