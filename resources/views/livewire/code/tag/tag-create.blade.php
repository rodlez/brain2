<div class="bg-white shadow rounded-xl mx-4">
    ...
    @if ($show % 2 != 0)
        <div class="mx-auto w-11/12 pt-4 pb-0">
            <div class="bg-zinc-100 text-black p-4 rounded-lg mx-2 border-double border-4 border-green-500">
                <p>Enter the name in the textbox.<br>To batch creation add more categories using <i class="fa-solid fa-circle-plus text-green-600 hover:text-green-500" title="Add new tag"></i> button.
                </p>
            </div>
        </div>
    @endif
    ...
    <div class="mx-auto w-11/12 py-4 px-2">
        <div>
            ...
            @if ($inputs->count() < 5)
                <button wire:click.prevent="add"><i class="fa-solid fa-circle-plus text-green-600 hover:text-green-500" title="Add new tag"></i></button>
            @else
