...
<x-app-layout>
    <div class="container max-w-6xl mx-auto px-6 py-4">
        ...
        <div class="flex flex-row justify-start items-center py-2 px-2 text-slate-400">
            ...
            <a href="/dashboard/code/tag/{{ $tag->id }}" class="px-2 hover:text-green-600">Info</a> /
            <a href="/dashboard/code/tag/edit/{{ $tag->id }}" class="px-2 font-bold text-black border-b-2 border-b-green-500">Edit</a>
        </div>
            ...
            <div class="mx-auto w-11/12 py-4 px-2">
                ...
                <form action="{{ route('codetag.update', $tag) }}" method="POST">
                    ...
                    <div class="flex flex-col justify-start items-start w-full sm:w-2/3 gap-4 py-2">
                        ...
                        <input name="name" id="name" type="text" value="{{ $tag->name }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-md rounded-lg w-2/3 pl-4 p-2 focus:ring-green-500 focus:border-green-500">
                    </div>
                    <!-- Errors -->
                    @error('name')
                        <div>
